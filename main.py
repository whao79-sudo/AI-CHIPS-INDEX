...
if prev_c and curr_o and not np.isnan(prev_c) and not np.isnan(curr_o):
gap = (curr_o - prev_c) / prev_c
if gap < -0.25:
adj *= prev_c / curr_o
print(" Adjust {} at {}: gap={:.1%}".format(code, sub.loc[i, "date"], gap))
if adj != 1.0:
for col in ["open","high","low","close"]:
sub.loc[i, col] = round(sub.loc[i, col] * adj, 2)
adjusted.append(sub)
...
def yp(v):
return p_ + ph - (v-mv)/rg*ph
cw = max(2, min(12, pw//n-1))
hw = max(1, cw//2)

def xp(i):
return p_ + i*pw//(n-1 if n>1 else 1)
...
yy = p_ + i*ph//4
v = mv + (1-i/4)*rg
gs += '<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="#222" stroke-width="0.5"/>'.format(p_, yy, p_+pw, yy)
gs += '<text x="{}" y="{}" fill="#666" font-size="11" text-anchor="end">{:.0f}</text>'.format(p_-5, yy+4, v)

sg += fill_area(sa_, sb_, "#2f4f4f")
sg += fill_area(sb_, sa_, "#8b4513")
sg += poly(sa_, "#2f4f4f", 1)
sg += poly(sb_, "#8b4513", 1)
sg += poly(ub, "#aa843e", 0.5, "dash")
sg += poly(lb, "#aa843e", 0.5, "dash")
sg += poly(boll, "#aa843e", 1)
sg += poly(ma300, "#ffd700", 2)
sg += poly(ub1, "#ffd700", 0.5, "dash")
sg += poly(lb1, "#ffd700", 0.5, "dash")
sg += poly(ub1a, "#4682b4", 1, "dash")
sg += poly(lb1a, "#4682b4", 1, "dash")
sg += poly(ub2, "#888", 0.5, "dash")
sg += poly(lb2, "#888", 0.5, "dash")
sg += poly(ema5, "#fff", 1.5)
sg += poly(ma50, "#0f0", 1.5)
sg += poly(ma100, "#f44", 2)
...
for i in range(n):
if i % 60 == 0 or i == n-1:
xt += '<text x="{}" y="{}" fill="#666" font-size="10" text-anchor="middle">{}</text>'.format(xp(i), p_+ph+20, ds[i])

box = '<rect x="{}" y="{}" width="{}" height="{}" fill="none" stroke="#444" stroke-width="1"/>'.format(p_, p_, pw, ph)
...
stats = ""
for l, v in [("最新点位", str(lt["index_value"])),
